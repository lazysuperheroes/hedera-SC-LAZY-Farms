/**
 * Batch check if spenders are approved for all NFT serials via LazyAllowanceUtility
...
const main = async () => {
	...
	const args = parseArgs(4, 'checkMultiNFTAllowanceAllSerials.js 0.0.CCC 0.0.TOKEN1,0.0.TOKEN2 0.0.OWNER1,0.0.OWNER2 0.0.SPENDER1,0.0.SPENDER2', [
		'BATCH CHECK VERSION',
		'CCC is the LazyAllowanceUtility address',
		'TOKEN1,TOKEN2,... is the NFT token list we are checking for all serials approval',
		'OWNER1,OWNER2,... is the owners of the NFT token(s)',
		'SPENDER1,SPENDER2,... is the spenders of the NFT tokens',
	]);
	...
	printHeader({
		scriptName: 'Check NFT Allowance All Serials (Batch)',
		env,
	...
	for (let i = 0; i < tokenList.length; i++) {
		console.log(
			...
			'All serials approved:', Boolean(approved[0][i]),
		);
	}
};
