...
const main = async () => {
	...
	const args = parseArgs(3, 'addCollectionToBoostLevel.js 0.0.BBB <rank> 0.0.GGGG1,0.0.GGGG2,0.0.GGGG3', [
		'0.0.BBB is the BoostManager contract to update',
		'<rank> is the boost level (0 - 5 or C|R|SR|UR|LR|SPE)',
		'0.0.GGGG is the collection to add to the boost level',
	]);
	...
	printHeader({
		scriptName: 'Add Collection to Boost Level',
		env,
	...
	for (const token of tokenList) {
		...
		console.log(`Gem ${token.toString()} added to Level ${lookupLevel(rank)}. Transaction ID:`, result[2]?.transactionId?.toString());
	}
};
