...
const main = async () => {
	...
	if (args.length < 1 || getArgFlag('h')) {
		console.log('Usage: node addAdminToMissionFactory.js 0.0.XXX');
		process.exit(1);
	}
	...
	const newUserAsEVM = await homebrewPopulateAccountEvmAddress(env, newUser);
	console.log('\n-Adding Admin:', newUser.toString(), 'EVM:', newUserAsEVM);

	const proceed = readlineSync.keyInYNStrict('Do you want to add the admin to mission factory?');
	...
	if (rslt[0]?.status?.toString() != 'SUCCESS') {
		console.log('Failed ot add Admin:', rslt);
		return;
	}

	console.log('Admin Added:', rslt[2].transactionId.toString());
